...
public class ModifyProductsController implements Initializable {
    ...
    @FXML
    private TextField txt_modifyProduct_Search;
    @FXML
    private TextField txt_modifyProduct_IDFixed;
    @FXML
    private TextField txt_modifyProduct_Name;
    @FXML
    private TextField txt_modifyProduct_Inv;
    @FXML
    private TextField txt_modifyProduct_Cost;
    @FXML
    private TextField txt_modifyProduct_Max;
    @FXML
    private TextField txt_modifyProduct_Min;
    @FXML
    private Button btn_modifyProduct_Save;
    @FXML
    private Button btn_modifyProduct_Cancel;
    @FXML
    ...
    private final int productIndex = productToModify();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        product = productInfo().get(productIndex);
        productID = productInfo().get(productIndex).getProductID();
        txt_modifyProduct_IDFixed.setText("Fixed: " + productID);
        txt_modifyProduct_Name.setText(product.getProductName());
        txt_modifyProduct_Inv.setText(Integer.toString(product.getProductInv()));
        txt_modifyProduct_Cost.setText(Double.toString(product.getProductCost()));
        txt_modifyProduct_Min.setText(Integer.toString(product.getProductMin()));
        txt_modifyProduct_Max.setText(Integer.toString(product.getProductMax()));

        productParts = product.getProductRelatedParts();
    ...
    private void modifyProductSearchHandler(ActionEvent event) {
        ...
        String searchParts = txt_modifyProduct_Search.getText().toLowerCase();
        ...
        try {
            ...
            for (Part part : Inventory.partsList) {
                if (part.getPartName().toLowerCase().equals(searchParts)) {
                    ...
                    txt_modifyProduct_Search.setText("");
    ...
    @FXML
    private void modifyProductDeleteHandler(ActionEvent event) {
    ...
    private void modifyProductSaveHandler(ActionEvent event) throws IOException {
        String productName = txt_modifyProduct_Name.getText();
        String productInv = txt_modifyProduct_Inv.getText();
        String productCost = txt_modifyProduct_Cost.getText();
        String productMin = txt_modifyProduct_Min.getText();
        String productMax = txt_modifyProduct_Max.getText();
        ...
        try {
            ...
            if (errorMessage.length() != 0) {
                ...
                product.setProductInv(Integer.parseInt(productInv));
                product.setProductMin(Integer.parseInt(productMin));
                product.setProductMax(Integer.parseInt(productMax));
                product.setProductRelatedParts(productParts);

                Stage stage;
                Parent root;
                stage = (Stage) btn_modifyProduct_Save.getScene().getWindow();
    ...
    private void modifyProductCancelHandler(ActionEvent event) throws IOException {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            Stage stage;
            Parent root;
            stage = (Stage) btn_modifyProduct_Cancel.getScene().getWindow();
            FXMLLoader loader = new FXMLLoader(getClass().getResource("MainScreen.fxml"));
            root = loader.load();
            Scene scene = new Scene(root);
            stage.setScene(scene);
